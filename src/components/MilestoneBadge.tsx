...
export function MilestoneBadge({
    ...
    switch (type) {
      case 'elevated':
        return <div className="w-full h-full flex items-center justify-center relative overflow-hidden rounded-full">
            ...
            <svg viewBox="0 0 24 24" className="w-6 h-6 md:w-8 md:h-8 text-emerald-600 fill-current" style={{
      ...
      case 'summit':
        return <div className="w-full h-full flex items-center justify-center relative overflow-hidden rounded-full">
            ...
            <span className="text-lg md:text-xl font-serif font-bold text-cyan-700">
              12
            </span>
          </div>;
      case '800club':
        return <div className="w-full h-full flex items-center justify-center relative overflow-hidden rounded-full bg-gradient-to-b from-yellow-300 to-yellow-500 border-2 border-yellow-200 shadow-inner">
            <div className="text-center">
              <div className="text-[10px] md:text-xs font-bold text-yellow-900 leading-none">
                800
              </div>
              <div className="text-[7px] md:text-[8px] font-bold text-yellow-800 uppercase tracking-wider">
                Club
              </div>
            </div>
          </div>;
  ...
  return <motion.div initial={{
    ...
    delay
  }} className="flex flex-col items-center gap-1 md:gap-2 min-w-[60px] md:min-w-[80px]">
      <div className={`w-12 h-12 md:w-16 md:h-16 rounded-full border-3 md:border-4 flex items-center justify-center shadow-md md:shadow-lg relative ${getBadgeStyle()}`}>
        {/* Shine effect */}
      ...
      <span className="text-[10px] md:text-xs font-medium text-slate-600 capitalize">
        {label}
      </span>
    </motion.div>;
}